...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        too_high = '14'
        valid_input = '5'
        allow(game_input).to receive(:gets).and_return(too_high, valid_input)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        a_symbol = '$'
        valid_input = '5'
        allow(game_input).to receive(:gets).and_return(too_high, a_symbol, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    game_min = 1
    game_max = 10
    subject(:verify_input_game) { described_class.new(game_min, game_max) }
    # Note: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        game_input = 5
        expect(verify_input_game.verify_input(game_min, game_max, game_input)).to eq(game_input)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        game_input = 100
        expect(verify_input_game.verify_input(game_min, game_max, game_input)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:display_game) { described_class.new(1, 10) }
    let(:bin_search) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(bin_search).to receive(:make_guess)
      allow(bin_search).to receive(:update_range)
      allow(display_game).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { display_game.display_turn_order(bin_search) }.to change { display_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(bin_search).to receive(:make_guess).once
      display_game.display_turn_order(bin_search)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(bin_search).to receive(:update_range).once
      display_game.display_turn_order(bin_search)
    end

...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        response = 4
        expect(game_guessing.make_guess).to eq(response)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      subject(:five_nine_game) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(five_nine_game.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:eight_nine_game) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(eight_nine_game.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:zero_three_game) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(zero_three_game.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:three_three_game) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(three_three_game.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:game_over_random) { double('random_number', value: 8) }
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over_game) { described_class.new(0, 9, game_over_random, 8) }
      ...
      it 'is game over' do
        expect(game_over_game.game_over?).to be(true)
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:not_game_over_game) { described_class.new(0, 9, game_over_random, 5) }
      it 'is not game over' do
        expect(not_game_over_game.game_over?).to be(false)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:same_guess_game) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        expect { same_guess_game.update_range }.to change { same_guess_game.min }.to(same_guess_game.max)
      end

      it 'does not update max' do
        expect { same_guess_game.update_range }.not_to change { same_guess_game.max }
      end
    end
  end
end
